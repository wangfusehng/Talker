...
class ConvNormRelu(nn.Module):
    ...
    def __init__(self,
        ...
        # 如果kernel_size和stride都为None，则根据是否下采样设置默认值
        ...
        # 如果padding为None，则根据kernel_size和stride的类型设置默认值
        if padding is None:
        ...
        # 如果residual为True，则根据是否下采样设置residual_layer
        if self.residual:
            if downsample:
                if type == '1d':
        ...
        # 将in_channels和out_channels乘以groups
        ...
        # 如果norm为'gn'，则使用GroupNorm；如果norm为'ln'，则使用LayerNorm
        if norm == 'gn':
            self.norm = nn.GroupNorm(2, out_channels)
        elif norm == 'ln':
            self.norm = nn.LayerNorm(out_channels)
        # 如果leaky为True，则使用LeakyReLU；否则使用ReLU
        if leaky:
    ...
    def forward(self, x, **kwargs):
        # 如果norm_type为'ln'，则先进行dropout，再进行conv，最后进行norm
        if self.norm_type == 'ln':
            out = self.dropout(self.conv(x))
            out = self.norm(out.transpose(1,2)).transpose(1,2)
        else:
            # 否则先进行norm，再进行dropout，最后进行conv
            out = self.norm(self.dropout(self.conv(x)))
        # 如果residual为True，则将residual加到out上
        if self.residual:
