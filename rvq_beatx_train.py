...
trainSet = CustomDataset(dataset_args,"train")
testSet = CustomDataset(dataset_args,"test")
...
elif args.body_part in "expression":
...
#     for i in joints:
#         face_body_mask.extend([i*6, i*6+1, i*6+2, i*6+3, i*6+4, i*6+5])
#     mask = face_body_mask + list(range(333, 433))#add expression
#     rec_mask = list(range(len(mask)))   
...
if args.mode == 'train':
    ...
    for nb_iter in range(1, args.warm_up_iter):
        ...
        # print(gt_motion.shape)#[batch_size,64,118]
    ...
    for nb_iter in range(1, args.total_iter + 1):
        ...
        gt_motion = next(train_loader_iter)
        # print(f"gt_motion shape: {gt_motion.shape}")#[batch_size,64,443]
        gt_motion = gt_motion[...,mask].cuda().float() # bs, seq_len, joints_dim, 
